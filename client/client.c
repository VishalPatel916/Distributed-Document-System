// Entry point and main interactive loop for the Client application.
...
static void handle_create(int sock) {
...
static void handle_read(int sock) {
    char* filename = strtok(NULL, " \n");
    if (filename == NULL) {
...
static void handle_write(int sock) {
    char* filename = strtok(NULL, " \n");
...
// Handle listing of registered users.
static void handle_list(int sock) {
    ...
    if (header.type == RES_LIST_HDR) {
        ...
        recv(sock, &list_hdr, sizeof(list_hdr), 0);
        printf("--- Registered Users (%d) ---\n", list_hdr.user_count);
        for (int i = 0; i < list_hdr.user_count; i++) { 
            Msg_List_Item item; 
...
static void handle_delete(int sock) {
    ...
    if (header.type == RES_OK) {
        ...
        printf("Error: Access denied (you are not the owner).\n");
    } else {
        printf("Error: Could not delete file.\n");
    }
}

// Handle file undo command.
static void handle_undo(int sock) {
    char* filename = strtok(NULL, " \n");
    if (filename == NULL) {
    ...
    if (header.type == RES_OK) {
        ...
        printf("Error: Could not undo file.\n");
    }
}

// Handle file stream command.
static void handle_stream(int sock) {
    ...
    if (header.type == RES_READ_LOCATION) { 
        ...
        printf("Error: Could not stream file.\n");
    }
}

// Handle viewing files command.
static void handle_view(int sock) {
    ...
    for (int i = 0; i < view_hdr.file_count; i++) {
        ...
        if (header.type == RES_VIEW_ITEM_SHORT) {
            ...
            Msg_Full_Metadata meta; 
            recv(sock, &meta, sizeof(meta), 0);
...
// Handle viewing file metadata information.
static void handle_info(int sock) {
    char* filename = strtok(NULL, " \n");
    if (filename == NULL) {
...
// Handle adding user access permissions to a file.
static void handle_add_access(int sock) {
    char* flag = strtok(NULL, " \n");
    char* filename = strtok(NULL, " \n");
    char* user = strtok(NULL, "\n");
    if (flag == NULL || filename == NULL || user == NULL) {
        printf("Usage: ADDACCESS -R|-W <filename> <username>\n");
        return;
    }
    while (*user == ' ') user++;
    size_t ulen = strlen(user);
    while (ulen > 0 && user[ulen - 1] == ' ') {
        user[--ulen] = '\0';
    }
    if (ulen == 0) {
        printf("Usage: ADDACCESS -R|-W <filename> <username>\n");
    ...
    if (header.type == RES_OK) {
        ...
        printf("Error: Could not add access.\n");
    }
}

// Handle removing user access permissions from a file.
static void handle_rem_access(int sock) {
    char* filename = strtok(NULL, " \n");
    char* user = strtok(NULL, "\n");
    ...
    if (header.type == RES_OK) {
        ...
        printf("Error: Could not remove access.\n");
    }
}

// Handle requesting access to a file.
static void handle_req_access(int sock, const char* username) {
    char* flag = strtok(NULL, " \n");
    char* filename = strtok(NULL, " \n");
    ...
    strncpy(req.filename, filename, MAX_FILENAME);
    strncpy(req.requesting_user, username, MAX_USERNAME);
    if (strcmp(flag, "-W") == 0) {
        req.requested_perm = READ_WRITE;
    } else {
        req.requested_perm = READ_ONLY;
    }
...
// Handle checking and processing access requests for owned files.
static void handle_check_requests(int sock) {
    Header header;
    ...
    if (header.type == RES_REQUEST_LIST) {
        ...
        if (hdr.request_count == 0) {
            ...
            for (int i = 0; i < hdr.request_count; i++) {
                char time_str[100];
                strftime(time_str, sizeof(time_str), "%Y-%m-%d %H:%M:%S", localtime(&items[i].timestamp));
...
// Handle remote command execution command.
static void handle_exec(int sock) {
    char* filename = strtok(NULL, " \n");
    if (filename == NULL) {
...
static void handle_create_folder(int sock) {
...
static void handle_move(int sock) {
    char* filename = strtok(NULL, " \n");
    char* foldername = strtok(NULL, " \n");
...
// Handle listing folder contents.
static void handle_view_folder(int sock) {
    char* foldername = strtok(NULL, " \n");
    if (foldername == NULL) {
...
// Handle creating a checkpoint for a file.
static void handle_checkpoint(int sock) {
    char* filename = strtok(NULL, " \n");
...
// Handle viewing a file checkpoint's content.
static void handle_view_checkpoint(int sock) {
    char* filename = strtok(NULL, " \n");
    char* tag = strtok(NULL, " \n");
    if (filename == NULL || tag == NULL) {
...
// Handle reverting a file to a checkpoint.
static void handle_revert(int sock) {
    char* filename = strtok(NULL, " \n");
    char* tag = strtok(NULL, " \n");
    if (filename == NULL || tag == NULL) {
...
// Handle listing checkpoints for a file.
static void handle_list_checkpoints(int sock) {
    char* filename = strtok(NULL, " \n");
    if (filename == NULL) {
