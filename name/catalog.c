// Implementation of Naming Server (NM) catalog management.
...
// Find file slot index in catalog using cache and hashmap.
...
// Find an empty slot in the file catalog.
int find_empty_file_slot() { 
    for (int i = 0; i < MAX_FILES_IN_SYSTEM; i++) { 
        if (file_catalog[i].active == 0) return i; 
    } 
    return -1; 
}

// Find the first available Storage Server.
int find_available_ss() { 
    for (int i = 0; i < MAX_CONNECTIONS; i++) { 
        if (ss_state[i].active) return i; 
    } 
    return -1; 
}

// Send standard OK acknowledgment header.
void send_ok_response(int sock) { 
    Header header; 
    header.type = RES_OK; 
    header.payload_size = 0; 
    if (send(sock, &header, sizeof(Header), 0) < 0) { 
        log_event("Failed to send OK response to socket %d", sock); 
    } 
}

// Clean up catalog entries and disconnect client/server.
...
// Fetch user permission level for a specific file.
...
int has_read_access(FileMetadata* meta, char* username) { 
    return get_permission(meta, username) >= READ_ONLY; 
}

// Check if user has write permission.
int has_write_access(FileMetadata* meta, char* username) { 
    return get_permission(meta, username) == READ_WRITE; 
}

// Send full file metadata structure over socket.
void send_full_metadata(int sock_fd, FileMetadata* meta, MessageType res_type) {
    Header hdr; 
    hdr.payload_size = sizeof(Msg_Full_Metadata); 
    hdr.type = res_type;
    ...
    msg.word_count = meta->word_count; 
    msg.char_count = meta->char_count;
    msg.last_modified = meta->last_modified; 
    msg.last_accessed = meta->last_accessed; 
    msg.access_count = meta->access_count;
    
    send(sock_fd, &hdr, sizeof(hdr), 0); 
    send(sock_fd, &msg, sizeof(msg), 0);
    
    if (msg.access_count > 0) { 
        send(sock_fd, meta->access_list, sizeof(AccessEntry) * msg.access_count, 0); 
    }
}

// Fetch script content from Storage Server.
char* get_file_content_from_ss(SSInfo* ss, char* filename) {
    ...
    if (inet_pton(AF_INET, ss->ip, &ss_addr.sin_addr) <= 0) {
        log_event("  -> EXEC: inet_pton() failed"); 
        close(ss_sock); 
        return NULL;
    }
    
    if (connect(ss_sock, (struct sockaddr *)&ss_addr, sizeof(ss_addr)) < 0) {
        log_event("  -> EXEC: connect() to SS failed"); 
        close(ss_sock); 
        return NULL;
    }
